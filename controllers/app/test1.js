...
const menu = async function (req, res) {
    ...
    let sql = mysql.format("SELECT 	sale_price,groupPic,custom_qua,item_cat,city_tax,state_tax,menugroup.id as groupid, mi.id as item_id,customizations,group_name,mi.item_name,mi.item_price,mi.item_pic,item_des,if(out_of_stock = 0 , item_quantity , 0) as item_quantity FROM menugroup LEFT JOIN menugroupitem as mi on mi.group_id = menugroup.id WHERE menugroup.restaurant_id = ? ORDER BY menugroup.id", [res_id]);
...
const getAdverts = async function (req, res) {
    ...
    if (random == 0)
        sql = `SELECT pa.id,pa.restaurant_id,restaurantpic, pa.pic, rs.name , rs.address , 3956 * 2 * ASIN(SQRT(POWER(SIN((${latitude} - abs(rs.latitude)) * pi() / 180 / 2),2) + COS(${latitude} * pi() / 180 ) * COS(abs(rs.latitude) * pi() / 180) * POWER(SIN((${longitude} - rs.longitude) * pi() / 180 / 2), 2) )) as distance FROM promo_advert as pa LEFT JOIN restaurant as rs ON pa.restaurant_id = rs.id WHERE start_date <= DATE(NOW()) and end_date >= DATE(NOW()) and pa.status = 1 having distance < ${config.distance} ORDER BY rand() limit 3`;

    if (random == 1)
        sql = `SELECT pa.id,pa.restaurant_id, pa.video ,restaurantpic, pa.video_thumb, rs.name , rs.address , 3956 * 2 * ASIN(SQRT(POWER(SIN((${latitude} - abs(rs.latitude)) * pi() / 180 / 2),2) + COS(${latitude} * pi() / 180 ) * COS(abs(rs.latitude) * pi() / 180) * POWER(SIN((${longitude} - rs.longitude) * pi() / 180 / 2), 2) )) as distance FROM promo_video as pa LEFT JOIN restaurant as rs ON pa.restaurant_id = rs.id WHERE start_date <= DATE(NOW()) and end_date >= DATE(NOW()) and pa.status = 1 having distance < ${config.distance} ORDER BY rand() limit 3`;
...
const addtocart = async function (req, res) {
    ...
    let deletesql = mysql.format('delete from cart where user_id = ? and ordered = 0', [user_id])
    let insertsql = mysql.format('insert into cart(	user_id,res_id,cart,city_tax,state_tax,total) values (?,?,?,?,?,?)', [user_id, res_id, JSON.stringify(cart), city_tax, state_tax, total]);
...
const getcart = async function (req, res) {
    ...
    let sql = mysql.format(`select r.min_order_value,r.max_order_value,r.cod,r.status as r_status,r.name as res_name,cart.*,${openclosefields},latitude,longitude from cart  left join res_openclose_time as roc on roc.res_id = cart.res_id left join restaurant as r on r.id = cart.res_id where user_id = ? and ordered = 0`, [user_id]);
    //console.log(sql)
    return await query_helper.runQuery(sql).then(response => {
...
const placeOrder = async function (req, res) {
    ...
    let { address, total, cart, cart_id, user_id, res_id, extra, token, delivery_mode, payment_mode, selectOffer, selectedtimeslot } = req.body
    ...
    if (items.length > 0) {
        ...
        return await query_helper.runMultiQuery(sqlArray).then(async response => {
            ...
            let { paymentStatus, paymentdata, Status, TotalAmmount } = await makePayment(req.body, extra.delivery_charge, OrderSubtotal, delivery_mode, payment_mode, token, stripe_acc)

            if (Status == 'paymentfailed')
                return res.send({ status: false, msg: paymentdata })

            //get time slots
            let delieverydate = null;
            let timeSlots = null
            ...
            //console.log(delieverydate, timeSlots)

            let ps = paymentStatus ? 1 : 0
            ...
            //insert sql 
            let insertsql = mysql.format('insert into orders(user_id,res_id,cart_id,order_hash,cart,total,city_tax,state_tax,payment_status,payment_data,order_by,status,delivery_mode,payment_mode,delivery_charge,order_code,delivered_by,discount,without_discount,delieverydate, timeSlots,contact_mode) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', [user_id, res_id, cart_id, order_hash, JSON.stringify(cart), TotalAmmount, extra.city_tax, extra.state_tax, ps, paymentdata, orderby, Status, delivery_mode, payment_mode, extra.delivery_charge, randomNo, delieved_by, TotalDiscount, subtotal, delieverydate, timeSlots, extra.contact_mode])

            return await query_helper.runQuery(insertsql).then(async resp => {
                //console.log(resp)
                if (resp && resp.affectedRows > 0) {
                    ...
                    await updateAfterOrder(cart_id, resp.insertId, useraddress, Status, response[0], cart)
                    ...
                    await query_helper.runMultiQuery([getuserEmail, getresOwnerEmail]).then(result => {
                        if (paymentStatus || payment_mode == 2) {
                            ...
                            //send email to user
                            mail_helper.mailer(
                                { orderid: resp.insertId, order_hash, email: useraddress.email, pref_lang: result[0][0] ? result[0][0].pref_lang : 'en', res_name },
                            ...
                            if (result[0][0] && result[0][0].device_token && result[0][0].device_token != null && result[0][0].device_token != '')
                                pushnotification.send_push(result[0][0].device_token, result[0][0].platform, 'Order Confirmation', `Your order is successfully created . Your Order Id is  ${resp.insertId}`, 'NewOrder', '', '');

                            //send email to owner 
                            mail_helper.mailer(
                                {
                                    orderid: resp.insertId, user: result[1], email: orderemail, pref_lang: result[1][0].pref_lang
                                },
                    ...
                    if (paymentStatus || payment_mode == 2) {
                        return res.send({ status: true, msg: "your order has been Confirmed", paymentStatus: true, order_id: resp.insertId, order_hash })
                    } else {
...
async function makePayment(body, Delievery_Charge, subtotal, delivery_mode, payment_mode, token, stripe_acc) {
...
module.exports = {
    ...
    getResMenu, claim, addtocart, getcart, placeOrder, clearCart, autoSearch, getOrderDetail, getongoingOrders, getOrderStages, getDiscounts, getDiscountsWithinLocation, getTimeslots, cancelOrder
};
